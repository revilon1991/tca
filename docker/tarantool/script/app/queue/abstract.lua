...
queue.driver = {
    ...
    fifo_replace = require('queue.abstract.driver.fifo_replace'),
    fifo_skip    = require('queue.abstract.driver.fifo_skip'),
...
function tube.ack_list(self, id_list)
    for _, id in pairs(id_list) do
        tube.ack(self, id)
    end
end

function tube.put_list(self, data_list)
    for _, value in pairs(data_list) do
        local data = value.data;
        local opts = value.options;

        tube.put(self, data, opts)
    end
end

function tube.take_list(self, limit, timeout)
    local task_list = {}

    for i = 1,limit do
        local task = tube.take(self, timeout)
        if task == nil then
            return task_list
        end
